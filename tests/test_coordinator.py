"""Tests for coordinated gateway updates."""

import logging
from datetime import datetime, timedelta
from unittest.mock import AsyncMock
from zoneinfo import ZoneInfo

from homeassistant.util import dt as dt_util
from pytest_homeassistant_custom_component.common import MockConfigEntry

from custom_components.nep_local.const import CONF_HOST, DOMAIN
from custom_components.nep_local.coordinator import NepDataUpdateCoordinator
from custom_components.nep_local.exceptions import NepResponseMissing
from custom_components.nep_local.models import (
    AggregateReading,
    GatewayInventory,
    InventoryModule,
    MinDatRecord,
    ModuleReading,
    ModuleStatus,
)


async def test_coordinator_keeps_partial_module_data(hass, caplog) -> None:
    module = InventoryModule(address="9", raw_id="0XAAA00050")
    client = AsyncMock()
    client.inventory.return_value = GatewayInventory("TESTGW000001", (module,))
    client.aggregate.return_value = AggregateReading(power_w=42)
    client.module.return_value = ModuleReading(
        raw_id=module.raw_id,
        address=module.address,
        power_w=42,
        status_code="8000",
        status=ModuleStatus.LOW_LIGHT,
    )
    client.min_dat.side_effect = NepResponseMissing("nighttime telemetry omitted")
    entry = MockConfigEntry(domain=DOMAIN, data={CONF_HOST: "192.0.2.10"})
    entry.add_to_hass(hass)

    coordinator = NepDataUpdateCoordinator(hass, entry, client)
    with caplog.at_level(logging.DEBUG):
        await coordinator.async_refresh()

    assert coordinator.last_update_success
    assert coordinator.data.aggregate.power_w == 42
    assert (
        coordinator.data.modules[module.raw_id].reading.status is ModuleStatus.LOW_LIGHT
    )
    assert coordinator.data.modules[module.raw_id].telemetry is None
    assert "Module telemetry endpoint failed with NepResponseMissing" in caplog.text
    assert module.raw_id not in caplog.text


async def test_coordinator_rejects_stale_min_dat_telemetry(hass, caplog) -> None:
    module = InventoryModule(address="9", raw_id="0XAAA00050")
    client = AsyncMock()
    client.inventory.return_value = GatewayInventory("TESTGW000001", (module,))
    client.aggregate.return_value = AggregateReading(power_w=42)
    client.module.side_effect = NepResponseMissing("live endpoint omitted")
    client.min_dat.return_value = MinDatRecord(
        timestamp=(dt_util.now() - timedelta(minutes=16)).replace(tzinfo=None),
        power_w=42,
        voltage_dc_v=37,
        voltage_ac_v=240,
        current_a=0.2,
        frequency_hz=60,
        temperature_c=30,
        energy_wh=100,
        rssi=-70,
        diagnostic_code="0",
        status_code="0000",
    )
    entry = MockConfigEntry(domain=DOMAIN, data={CONF_HOST: "192.0.2.10"})
    entry.add_to_hass(hass)

    coordinator = NepDataUpdateCoordinator(hass, entry, client)
    with caplog.at_level(logging.DEBUG):
        await coordinator.async_refresh()

    assert coordinator.last_update_success
    assert coordinator.data.modules[module.raw_id].telemetry is None
    assert "stale sample" in caplog.text


def test_telemetry_freshness_uses_elapsed_time_across_dst() -> None:
    timezone = ZoneInfo("America/Chicago")

    def record_at(timestamp: datetime) -> MinDatRecord:
        return MinDatRecord(
            timestamp=timestamp,
            power_w=1,
            voltage_dc_v=1,
            voltage_ac_v=1,
            current_a=1,
            frequency_hz=60,
            temperature_c=1,
            energy_wh=1,
            rssi=1,
            diagnostic_code="0",
            status_code="0000",
        )

    assert NepDataUpdateCoordinator._telemetry_is_fresh(
        record_at(datetime(2026, 3, 8, 1, 55)),
        datetime(2026, 3, 8, 3, 5, tzinfo=timezone),
    )
    assert NepDataUpdateCoordinator._telemetry_is_fresh(
        record_at(datetime(2026, 11, 1, 1, 55)),
        datetime(2026, 11, 1, 2, 5, tzinfo=timezone),
    )
    assert NepDataUpdateCoordinator._telemetry_is_fresh(
        record_at(datetime(2026, 7, 21, 12, 5)),
        datetime(2026, 7, 21, 12, 0, tzinfo=timezone),
    )
    assert not NepDataUpdateCoordinator._telemetry_is_fresh(
        record_at(datetime(2026, 7, 21, 12, 5, 1)),
        datetime(2026, 7, 21, 12, 0, tzinfo=timezone),
    )
