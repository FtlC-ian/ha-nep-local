...
from .models import AggregateReading, InventoryModule, ModuleStatus
...
MODULE_SENSORS = (
    ...
    NepModuleSensorDescription(
        ...
        suggested_display_precision=0,
        requires_live=True,
        value_fn=lambda data: data.reading.today_wh if data.reading else None,
    ),
    NepModuleSensorDescription(
        key="lifetime_energy",
    ...
    NepModuleSensorDescription(
        key="ac_voltage",
        translation_key="module_ac_voltage",
        device_class=SensorDeviceClass.VOLTAGE,
        native_unit_of_measurement=UnitOfElectricPotential.VOLT,
        state_class=SensorStateClass.MEASUREMENT,
        requires_telemetry=True,
        value_fn=lambda data: data.telemetry.voltage_ac_v if data.telemetry else None,
    ),
    NepModuleSensorDescription(
        key="current",
    ...
    NepModuleSensorDescription(
        key="frequency",
        translation_key="module_frequency",
        device_class=SensorDeviceClass.FREQUENCY,
        native_unit_of_measurement=UnitOfFrequency.HERTZ,
        state_class=SensorStateClass.MEASUREMENT,
        requires_telemetry=True,
        value_fn=lambda data: data.telemetry.frequency_hz if data.telemetry else None,
    ),
    NepModuleSensorDescription(
        key="temperature",
        ...
        state_class=SensorStateClass.MEASUREMENT,
        requires_telemetry=True,
        value_fn=lambda data: data.telemetry.temperature_c if data.telemetry else None,
    ),
    NepModuleSensorDescription(
        key="status",
        translation_key="module_status",
        device_class=SensorDeviceClass.ENUM,
        options=tuple(status.value for status in ModuleStatus),
...
async def async_setup_entry(
    ...
    def add_discovered() -> None:
        new_modules = [
            ...
            if module.raw_id not in known
        ]
        if not new_modules:
            return
        known.update(module.raw_id for module in new_modules)
        async_add_entities(
...
class NepGatewaySensor(CoordinatorEntity[NepDataUpdateCoordinator], SensorEntity):
    ...
    def __init__(
        ...
        self._attr_device_info = DeviceInfo(
            ...
            if str(entry.data["host"]).startswith(("http://", "https://"))
            else f"http://{entry.data['host']}",
        )
